...
> Goal: deliver an **incremental, performant, pragmatic** Hindley–Milner (HM) type inference and checking engine for Python that integrates with modern editor tooling via the Language Server Protocol (LSP). The system should support Python’s dynamic features thoughtfully, interoperate with `typing` hints, and scale to multi-file projects.

## Why HM for Python?

- **Pros:** principled inference (no annotations required), compositional reasoning, strong guarantees, fast unification-based algorithms (Algorithm W family).
- **Cons (Python):** pervasive dynamism (monkey-patching, `__getattr__`, metaclasses, duck typing, runtime reflection), nominal & structural patterns mixed, subtyping-ish expectations (`None`, unions, protocols), first-class classes & functions, decorators, generators, async, pattern matching (PEP 634).

**Design stance:** **HM core + pragmatic extensions**, with a **gradual boundary** to accommodate Python idioms and annotations:
...
- `crates/pyts_parser`: Tree-sitter bindings, Python grammar hooks, CST→AST adapters.
- `crates/types_core`: type representations, substitutions, unifier, generalization, kinding.
...
- **Entailment engine** for protocol satisfaction: solve `HasAttr(τ, "a", τa)` by:
    ...
    - Else generate **deferred constraint** resolved after more info arrives (important for incremental + multi-file).
...
**Transport:** Implement an LSP server (e.g., with `tower-lsp` or manual) but **use `lsp-types`** for request/response models.
