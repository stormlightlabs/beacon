        ...
        self.documents.get_document(uri, |doc| {
            ...
            match node.kind() {
                "identifier" => {
                    ...
                    if identifier_text.starts_with("__")
                        && identifier_text.ends_with("__")
            ...
            if let Some(ref python) = self.interpreter_path {
                ...
                match introspection::introspect_sync(python, &module, &symbol) {
                    ...
                    Err(e) => {
                        ...
                        {
                            let mut value =
                                format!("**Import** `{name}` from module `{module}`\n\n**Module documentation:**\n\n");
                            let parsed = parse_docstring(&doc);
                            let rendered = parsed.to_markdown();
                            if !rendered.trim().is_empty() {
                                value.push_str(rendered.trim());
            ...
            body: vec![AstNode::FunctionDef {
                ...
                args: vec![
                    Parameter {
                        ...
                        default_value: None,
                        kind: beacon_parser::ParameterKind::PositionalOrKeyword,
                        end_col: 1,
                        end_line: 16,
                    },
                    Parameter {
